...
import { Grid, Box, Typography, Card, CardMedia, Button } from '@mui/material';
...
const AdminMinting = ({
  ...
  const mintNFT = async () => {
    if (!loading && wallet && !wrongNetwork) {
      ...
      try {
        let tx = await factoryContract.mintGenerator(nftName, {
          value: ethers.utils.parseEther(`${nftPrice / ETHUSDConversionRate + 0.000000000001}`),
        });
        await tx.wait(1);
      ...
      handleAlerts(
        'Please, change to the right network (click on the top right red button)',
        'warning',
      );
    }
      ...
      if (!loading && generatorId && selectedProduct) {
        ...
        if (selectedAll) {
          ...
          if (there_is_product_selected) {
            handleAlerts(
              'Withdrawed successfully from products (IDs): ' +
                products_str.substr(0, products_str.length - 2),
              'success',
            );
          } else {
  ...
  const withdrawAndDelete = async () => {
    try {
      if (!loading && generatorId && selectedProduct) {
        ...
        handleAlerts(
          'Successfully deleted products (IDs): ' + products_str.substr(0, products_str.length - 2),
          'success',
        );
      } else if (loading) {
        handleAlerts('Loading... Cannot execute while loading', 'info');
    ...
    if (!loading && newProductName && parseFloat(newProductPrice) >= 0 && newProductPrice) {
      try {
        ...
        const tx = await generatorContract.addProduct(
          newProductName,
          ethers.utils.parseEther(newProductPrice),
        );
        await tx.wait(1);
    ...
    if (!loading && generatorId && parseFloat(productNewPrice) >= 0 && productNewPrice !== '') {
      ...
      try {
        const tx = await generatorContract.changeProductPrice(
          productId,
          ethers.utils.parseEther(productNewPrice),
        );
        await tx.wait(1);
      ...
      >
        ...
        >
          ...
          <div id="adminTop">
            ...
            >
              <Typography sx={{ fontSize: '60px', fontFamily: 'Nunito' }}>Admin</Typography>
          ...
          <div id="ManageNFTs">
            ...
            >
              ...
              <Box
                sx={{ display: 'flex', flexGrow: 1, justifyContent: 'center', background: 'none' }}
              >
            ...
            >
              ...
              >
                <Typography sx={{ fontSize: '30px', marginLeft: 3, marginTop: 3 }}>
                  1. Make sure your wallet is connected(Top right of page, orange button that says
                  "CONNECT").
                </Typography>
                ...
                <Typography sx={{ fontSize: '30px', marginLeft: 3, marginTop: 2 }}>
                  3. Once you've connected your wallet, copy paste the localhost link next to the
                  button in a new tab to access the light picker page.
                </Typography>
                <Typography sx={{ fontSize: '30px', marginLeft: 3, marginTop: 2 }}>
