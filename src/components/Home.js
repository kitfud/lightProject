...
const Home = ({ handleAlerts, updateGeneratorList, updateProductList }) => {
  ...
  const sendDataFunc = async () => {
    if (typeof port === 'undefined' && socket && typeof status === 'undefined') {
      await socket.emit('user request', {
        data: RGBColorString,
        address: refAddress,
  ...
  useEffect(() => {
    if (selectedProductContract !== undefined && firstTimeListener) {
      selectedProductContract.on(
        'Deposit',
        async (payee, value, time, currentContractBalance, event) => {
  ...
  const UserSelectNFT = () => {
    return (
      <Box sx={{}}>
        <Typography sx={{ m: 1, fontFamily: 'Nunito' }}>Select NFT</Typography>
  ...
  const UserSelectProduct = () => {
    return (
      <Box sx={{}}>
        <Typography sx={{ m: 1, fontFamily: 'Nunito' }}>Select Product</Typography>
        <FormControl sx={{ m: 1, minWidth: 300 }}>
          <Select
